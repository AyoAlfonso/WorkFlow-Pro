...
class CreateJournalEntries < ActiveRecord::Migration[6.0]
  ...
  def data
    ...
    QuestionnaireAttempt.find_each do |qa|
      ...
      JournalEntry.create!(generated_from_type: qa.class.name, generated_from_id: qa.id, body: parsed_body, user_id: qa.user_id, created_at: qa.completed_at, title: qa.questionnaire.name) if parsed_body.present?
    end
  end
end
