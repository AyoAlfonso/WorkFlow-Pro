class User < ApplicationRecord
  ...
  def current_daily_log
    if self.persisted?
      daily_logs.select(:id, :work_status, :create_my_day, :evening_reflection, :starting_mip_count).where(log_date: Date.today).first_or_create(starting_mip_count: self.todays_priorities.count)
    end
  end
