class MeetingInstanceFinderService < ApplicationService
  ...
  def call
    raise ArgumentError("Invalid meeting instance search parameters") if @meeting_template.blank? || @meeting_scope.blank? || @on_date_time.blank?
    ...
    when "forum_monthly", "personal_monthly"
      for_month_of_date(on_date_time)
    else
      raise ArgumentError("Meeting type not implemented for instance search")
    end
