class Api::GoalsController < Api::ApplicationController
  ...
  def index
    @company = Company.find(current_user.current_selected_company_id)
    ...
    @company_goals = policy_scope(AnnualInitiative).user_current_company(current_user).for_company_current_year_and_future(@company_current_fiscal_year)
