class Api::ScorecardLogsController < Api::ApplicationController
  ...
  def create
   manual_input_date =  Date.strptime(params[:manual_input_date], '%Y-%m-%d').to_date 
    if manual_input_date.present? && manual_input_date >= current_company.current_fiscal_start_date
     ...
     fiscal_quarter_start_date_closest_sunday = fiscal_quarter_start_date.sunday? ? fiscal_quarter_start_date : fiscal_quarter_start_date.next_occurring(:sunday)
        13.times do |weekly_index|
            if  manual_input_date <= fiscal_quarter_start_date_closest_sunday + (1.week * weekly_index) && manual_input_date >= fiscal_quarter_start_date + (1.week * weekly_index)
                ...
                break
              elsif manual_input_date > fiscal_quarter_start_date_closest_sunday + (1.week * weekly_index) && manual_input_date >= fiscal_quarter_start_date + (1.week * weekly_index) && manual_input_date < fiscal_quarter_start_date
                 params[:scorecard_log][:fiscal_quarter] = @quarter_index 
                 params[:scorecard_log][:week] = ((quarter_index)* 13) + (weekly_index+1)
                 params[:scorecard_log][:fiscal_year] = fiscal_quarter_start_date.year
                  break
     ...
     fiscal_quarter_start_date_closest_sunday = fiscal_quarter_start_date.sunday? ? fiscal_quarter_start_date : fiscal_quarter_start_date.next_occurring(:sunday)
        13.times do |weekly_index|
            if  manual_input_date <= fiscal_quarter_start_date_closest_sunday + (1.week * weekly_index) && manual_input_date >= fiscal_quarter_start_date + (1.week * weekly_index)
                ...
                params[:scorecard_log][:fiscal_year] = fiscal_quarter_start_date.year
                break
            elsif manual_input_date > fiscal_quarter_start_date_closest_sunday + (1.week * weekly_index) && manual_input_date >= fiscal_quarter_start_date + (1.week * weekly_index) && manual_input_date < fiscal_quarter_start_date_new_year
                 @quarter_index = quarter_index + 1
                 @weekly_index = weekly_index 
                  params[:scorecard_log][:fiscal_quarter] = @quarter_index 
                  params[:scorecard_log][:week] = ((quarter_index)* 13) + (weekly_index+1)
    ...
    render json: { scorecard_log: @scorecard_log, kpi: @kpi.as_json(), 
      status: :ok }
  end
